...
const USER_AGENT =
  "Vestige/1.0 (educational portfolio project; +https://github.com/AiDv11/history-bot)";
 ...
 *
 * `departments: null` disables artifacts for that era entirely. No current era
 * needs it, but the escape hatch stays: it was added for WWII, where the Met —
 * an art museum with essentially no Second World War holdings — cheerfully
 * returned a portrait from 1866 for "Operation Barbarossa". If an era is ever
 * added that the collection can't evidence, disable it rather than shipping
 * wrong objects. A wrong artifact is worse than none, because the whole point
 * of this feature is that the evidence is real.
 */
const ERA_SOURCES = {
  all: {
    hint: "",
  ...
  islamic: { hint: "islamic", departments: ["Islamic Art"] },
  song: { hint: "song dynasty", departments: ["Asian Art"] },
};
 ...
 *
 * @param {{hint: string, departments: string[]|null}|undefined} source
 * @returns {Promise<Array>} at most `limit` objects, best match first
 */
export async function findArtifacts(source, question, { limit = 3, timeoutMs = 12_000 } = {}) {
  // `source` is `{ hint, departments }` — from ERA_SOURCES for a built-in era,
  // or built from the single Met department a custom era was validated against.
  if (!source?.departments) return []; // era has no usable holdings

  const query = toQuery(question, source.hint);
  if (!query) return [];

  const key = `${source.departments.join("|")}:${query}`;
